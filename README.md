...
### 核心监控
- 实时行情监控（腾讯财经 API + WebSocket 推送）+ 刷新间隔可调且持久化
- K线烛形图 + 技术指标（MA/MACD/RSI/KDJ/BOLL）
- 股票分组/标签管理（一键打标签覆盖49只 + 手动编辑 + 分组删除）
- JWT 用户认证 + Token 黑名单

### AI 智能选股
- **一键选股**：AI 自动分析全市场，综合技术面+资金面+板块面推荐 8 只短线标的（自动过滤科创板688xxx）
...
| K线数据 | 百度股市通 | 日/周/月线 |
| 全市场情绪 | 监控列表实时计算 | 东方财富 push2 不可用时的降级方案 |
